...
*   Настраиваем PostgreSQL:
    ...
    -   Сервис берёт настройки БД через переменные окружения, поэтому рекомендуется использовать специальный скрипт (например [такой]() - ВЕРСИЯ ДЛЯ ОЗНАКОМЛЕНИЯ, НЕ ЗАБЫВАЕМ ИЗМЕНЯТЬ ДАННЫЕ) и инициилизировать переменные окручения через `. setup.sh`
